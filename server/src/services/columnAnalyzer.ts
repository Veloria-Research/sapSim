...
export class ColumnAnalyzer {
  ...
  async analyzeAllColumns(
    ...
    for (const table of tableStructures) {
      for (const field of table.fields) {
        const analysis = await this.analyzeColumn(
          table,
          field,
          table.sampleData || []
        );
  ...
  async analyzeColumn(
    ...
    const uniqueValueCount = new Set(
      sampleValues.filter((v) => v !== null && v !== undefined)
    ).size;
    const nullCount = sampleValues.filter(
      (v) => v === null || v === undefined
    ).length;
    const nullPercentage =
      sampleData.length > 0 ? (nullCount / sampleData.length) * 100 : 0;
    ...
    if (dataType.includes("VARCHAR") || dataType.includes("CHAR")) {
      ...
      if (stringValues.length > 0) {
        ...
        if (
          ...
          uniqueValues.length < stringValues.length * 0.5
        ) {
...
Respond in JSON format:
{
  ...
  "description": "..."
    ...
    try {
      ...
      console.error(
        `Error analyzing column ${tableName}.${field.name}:`,
        error
      );

      // Fallback analysis
      return {
        semanticType: this.inferSemanticType(
          field.name,
          field.type,
          sampleValues
        ),
  ...
  private inferSemanticType(
    ...
    // Common patterns
    if (name.includes("id") || name.includes("nr") || name.includes("num"))
      return "identifier";
