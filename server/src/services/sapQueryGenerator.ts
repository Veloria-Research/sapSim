...
export class SAPQueryGenerator {
  ...
  private initializeSAPTableDefinitions() {
    ...
    this.sapTableDefinitions.set('VBAK', {
      ...
      businessPurpose: 'Contains header information for sales orders, quotations, and contracts',
      columns: [
        { name: 'VBELN', type: 'VARCHAR(10)', semanticType: 'identifier', description: 'Sales Document Number', businessContext: 'Unique identifier for sales documents', sampleValues: ['5000001234', '5000001235'], isKey: true, isForeignKey: false },
        { name: 'AUART', type: 'CHAR(4)', semanticType: 'category', description: 'Sales Document Type', businessContext: 'Defines the type of sales document (order, quotation, etc.)', sampleValues: ['OR', 'QT', 'CR'], isKey: false, isForeignKey: false },
        { name: 'ERDAT', type: 'DATE', semanticType: 'date', description: 'Created On', businessContext: 'Date when the sales document was created', sampleValues: ['2025-01-15', '2025-01-16'], isKey: false, isForeignKey: false },
        { name: 'KUNNR', type: 'VARCHAR(10)', semanticType: 'identifier', description: 'Sold-to Party', businessContext: 'Customer who placed the order', sampleValues: ['0000500000', '0000500001'], isKey: false, isForeignKey: true, referencedTable: 'KNA1', referencedColumn: 'KUNNR' },
        { name: 'VKORG', type: 'VARCHAR(4)', semanticType: 'organization', description: 'Sales Organization', businessContext: 'Sales organization responsible for the sale', sampleValues: ['1000', '2000'], isKey: false, isForeignKey: false }
      ]
    });
    ...
    this.sapTableDefinitions.set('VBAP', {
      ...
      businessPurpose: 'Contains line item details for sales documents including materials and quantities',
      columns: [
        { name: 'VBELN', type: 'VARCHAR(10)', semanticType: 'identifier', description: 'Sales Document Number', businessContext: 'Links to sales document header', sampleValues: ['5000001234', '5000001235'], isKey: true, isForeignKey: true, referencedTable: 'VBAK', referencedColumn: 'VBELN' },
        { name: 'POSNR', type: 'VARCHAR(6)', semanticType: 'identifier', description: 'Sales Document Item', businessContext: 'Line item number within the sales document', sampleValues: ['000010', '000020'], isKey: true, isForeignKey: false },
        { name: 'MATNR', type: 'VARCHAR(18)', semanticType: 'identifier', description: 'Material Number', businessContext: 'Product being sold', sampleValues: ['000000000000100000', '000000000000100001'], isKey: false, isForeignKey: true, referencedTable: 'MARA', referencedColumn: 'MATNR' },
        { name: 'KWMENG', type: 'DECIMAL(15,3)', semanticType: 'quantity', description: 'Cumulative Order Quantity', businessContext: 'Total quantity ordered for this item', sampleValues: [10.000, 25.500], isKey: false, isForeignKey: false },
        { name: 'WERKS', type: 'VARCHAR(4)', semanticType: 'organization', description: 'Plant', businessContext: 'Manufacturing or distribution plant', sampleValues: ['1000', '2000'], isKey: false, isForeignKey: false }
      ]
    });
    ...
    this.sapTableDefinitions.set('MARA', {
      ...
      businessPurpose: 'Contains basic material information including material type, group, and unit of measure',
      columns: [
        { name: 'MATNR', type: 'VARCHAR(18)', semanticType: 'identifier', description: 'Material Number', businessContext: 'Unique identifier for materials/products', sampleValues: ['000000000000100000', '000000000000100001'], isKey: true, isForeignKey: false },
        { name: 'MTART', type: 'CHAR(4)', semanticType: 'category', description: 'Material Type', businessContext: 'Categorizes materials (finished goods, raw materials, etc.)', sampleValues: ['FERT', 'ROH', 'HALB'], isKey: false, isForeignKey: false },
        { name: 'MATKL', type: 'VARCHAR(9)', semanticType: 'category', description: 'Material Group', businessContext: 'Groups materials for reporting and analysis', sampleValues: ['UBHIOE', 'FR3AWXF'], isKey: false, isForeignKey: false },
        { name: 'MEINS', type: 'CHAR(3)', semanticType: 'unit', description: 'Base Unit of Measure', businessContext: 'Primary unit for measuring the material', sampleValues: ['KG', 'LTR', 'EA'], isKey: false, isForeignKey: false },
        { name: 'LAEDA', type: 'DATE', semanticType: 'date', description: 'Date of Last Change', businessContext: 'When the material master was last modified', sampleValues: ['2025-08-07', '2025-07-24'], isKey: false, isForeignKey: false }
      ]
    });
    ...
    this.sapTableDefinitions.set('KNA1', {
      ...
      businessPurpose: 'Contains general customer information including name, address, and region',
      columns: [
        { name: 'KUNNR', type: 'VARCHAR(10)', semanticType: 'identifier', description: 'Customer Number', businessContext: 'Unique identifier for customers', sampleValues: ['0000500000', '0000500001'], isKey: true, isForeignKey: false },
        { name: 'LAND1', type: 'CHAR(2)', semanticType: 'location', description: 'Country Key', businessContext: 'Country where customer is located', sampleValues: ['US', 'DE', 'UZ'], isKey: false, isForeignKey: false },
        { name: 'ORT01', type: 'VARCHAR(25)', semanticType: 'location', description: 'City', businessContext: 'Customer city', sampleValues: ['Tremblayworth', 'New Nella'], isKey: false, isForeignKey: false },
        { name: 'NAME1', type: 'VARCHAR(35)', semanticType: 'name', description: 'Name 1', businessContext: 'Customer company or person name', sampleValues: ['Berge, Stiedemann and Wisozk', 'Witting - King'], isKey: false, isForeignKey: false },
        { name: 'REGIO', type: 'VARCHAR(3)', semanticType: 'location', description: 'Region', businessContext: 'State or region code', sampleValues: ['DE', 'NH', 'CT'], isKey: false, isForeignKey: false }
      ]
    });
  }
  ...
  async generateSAPQuery(request: SAPQueryRequest): Promise<SAPQueryResult> {
    try {
      ...
      const businessIntent = await this.analyzeSAPBusinessIntent(request.prompt);
      
      // Step 2: Find relevant SAP tables and relationships
      const relevantContext = await this.findRelevantSAPContext(request.prompt, businessIntent);
      
      // Step 3: Generate the SQL query with SAP business logic
      const sqlResult = await this.generateSAPSQL(request.prompt, relevantContext, request);
      
      // Step 4: Validate against SAP business rules (legacy validation)
      const validation = await this.validateSAPQuery(sqlResult.sql, relevantContext);
      
      // Step 5: Enhanced validation using ValidatorAgent
      const groundTruthGraph = await this.validatorAgent.getGroundTruthForValidation();
      let validationResult: ValidationResult | null = null;
      
      if (groundTruthGraph) {
        validationResult = await this.validatorAgent.validateQuery(sqlResult.sql, groundTruthGraph, request.businessContext);
      }
      
      // Step 6: Calculate complexity and determine SAP modules
      const complexity = this.calculateSAPComplexity(sqlResult.sql, relevantContext);
      const sapModules = this.determineSAPModules(relevantContext.tables);
      
      // Combine validation results
      const finalValidationStatus = validationResult?.isValid && validation.isValid ? 'valid' : 
                                   ((validationResult?.warnings?.length || 0) > 0 || validation.hasWarnings ? 'warning' : 'invalid');
      const allErrors = [...(validation.errors || []), ...(validationResult?.errors || [])];
      
      const result: SAPQueryResult = {
        sql: sqlResult.sql,
        confidence: Math.min(sqlResult.confidence, validationResult?.confidence || 1.0),
      ...
      throw new Error(`Failed to generate SAP query: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async analyzeSAPBusinessIntent(prompt: string): Promise<{
    type: 'sales_analysis' | 'material_inquiry' | 'customer_data' | 'financial_report' | 'procurement' | 'complex_join';
    entities: string[];
  ...
  private async findRelevantSAPContext(prompt: string, intent: any): Promise<{
    ...
    if (prompt.toLowerCase().includes('sales') || prompt.toLowerCase().includes('order')) {
    ...
    if (prompt.toLowerCase().includes('material') || prompt.toLowerCase().includes('product')) {
    ...
    if (relevantTables.some(t => t.name === 'VBAK') && relevantTables.some(t => t.name === 'VBAP')) {
      relationships.push({
        leftTable: 'VBAK',
        leftColumn: 'VBELN',
        rightTable: 'VBAP',
    ...
    if (relevantTables.some(t => t.name === 'VBAP') && relevantTables.some(t => t.name === 'MARA')) {
      relationships.push({
        leftTable: 'VBAP',
        leftColumn: 'MATNR',
        rightTable: 'MARA',
    ...
    if (relevantTables.some(t => t.name === 'VBAK') && relevantTables.some(t => t.name === 'KNA1')) {
      relationships.push({
        leftTable: 'VBAK',
        leftColumn: 'KUNNR',
        rightTable: 'KNA1',
  ...
  ): Promise<{ sql: string; confidence: number; explanation: string; businessLogic: string }> {
...
SELECT 
    VBAK.VBELN AS Sales_Document_Number,
...
Return the response in this exact JSON format:
{
  ...
  "businessLogic": "Business logic explanation"
    ...
    try {
      ...
      context.tables.forEach(table => {
        ...
        const lowercaseRegex = new RegExp(`(?<!")\\b${table.name.toLowerCase()}\\b(?!")`, 'gi');
        ...
        table.columns.forEach(column => {
          const columnRegex = new RegExp(`(?<!")\\b${column.name.toLowerCase()}\\b(?!")`, 'gi');
  ...
  private generateFallbackSAPQuery(
    prompt: string,
    context: { tables: SAPTableContext[]; relationships: SAPRelationship[] }
  ): { sql: string; confidence: number; explanation: string; businessLogic: string } {
    
    if (context.tables.length === 0) {
      return {
        sql: "SELECT 'No relevant tables found' AS message;",
        confidence: 0.1,
        explanation: "No relevant SAP tables could be identified for this query.",
    ...
    context.tables.forEach(table => {
      table.columns.slice(0, 3).forEach(col => {
        columns.push(`    "${table.name}".${col.name} AS ${col.description.replace(/\s+/g, '_')}`);
      });
    });
  ...
  private async validateSAPQuery(sql: string, context: any): Promise<{
    isValid: boolean;
    hasWarnings: boolean;
    errors: string[];
  }> {
  ...
  private calculateSAPComplexity(sql: string, context: any): 'simple' | 'medium' | 'complex' {
    const sqlUpper = sql.toUpperCase();
    let complexity = 0;
